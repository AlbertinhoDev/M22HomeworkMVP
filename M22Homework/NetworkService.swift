    ...
    func getFilms(nameFilmForSearch: String, complition: @escaping (Result<[Model]?, Error>) -> Void) {
        ...
        URLSession.shared.dataTask(with: request) { data, _, error in
            ...
            do {
                ...
                for index in obj.films {
                    var model = Model(nameRu: index.nameRu ?? "",
                                      nameEn: index.nameEn ?? "",
                                      type: index.type ?? "",
                                      year: index.year ?? "",
                                      description: index.description ?? "",
                                      filmLength: index.filmLength ?? "",
                                      rating: index.rating ?? "",
                                      posterUrlPreview: nil) // Изначально присваиваем nil
                    if index.posterUrlPreview != nil {
                        group.enter() // Увеличиваем счетчик для dispatch group
                        self.getImage(urlString: index.posterUrlPreview!) { image in
